...
const ChatInfo = () => {
    ...
    return (
        <div className='w-[19%] h-[97%] flex flex-col space-y-2'>
            ...
            <div className='bg-white h-full rounded-3xl p-4'>
                {loading ? <Loader /> : activeChat && !sidebarOption.users ?
                    <>
                        <h2 className="text-xl font-bold mb-1">{participants.length}{" "} Members</h2>
                        {participants.length > 0 && participants?.map((participant, index) => (
                            <div className="p-2 border-b flex items-center">
                                <div className="w-8 h-8 rounded-full bg-gray-300"></div>
                                <div className="ml-4 flex justify-between w-full items-center">
                                    <p className="font-bold">{participant?.lastName
                                        ?
                                        `${participant?.firstName}${" "}${participant?.lastName}`
                                        :
                                        participant?.firstName}
                                    </p>
                                    <span className='text-[12px] text-gray-400'>{currentUser.uid === participant.uid && "You"}</span>
                                </div>
                            </div>
                        ))}
                    </> :
